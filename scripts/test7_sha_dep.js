...
async function main() {
    ...
    // Deploy EthSha contract
    const EthShaFactory = await ethers.getContractFactory("EthSha");
    const EthSha = await EthShaFactory.deploy(verifier1Address, verifier2Address, poseidon2Address, INITIAL_ROOT);
    await EthSha.waitForDeployment();
    const EthShaAddress = await EthSha.getAddress();
    console.log("EthSha deployed at:", EthShaAddress);

    const insertionCount = INSERTION_COUNT;
    const mmrGasUsed = [];

    for (let i = 0; i < insertionCount; i++) {
        const txResponse = await EthSha.deposit(SAMPLE_COMMITMENT, zkProof, SAMPLE_DEPOSIT_VALUE, { value: SAMPLE_ETHER_VALUE });
        const receipt = await txResponse.wait();
        mmrGasUsed.push(receipt.gasUsed.toString());
        printProgress(i + 1, insertionCount, "EthSha");
    }

    console.log("EthSha - Gas used per insert:", mmrGasUsed);
    ...
    const outputFilePath = path.join(__dirname, "../data/sha_dep_16_31_gas_used.json");
