...
// Configuration
// ---------------------------
const INSERTION_COUNT = 8;            // the gas cost of IMT insertation is nearly constant so 8 is enough to get a good average
...
const SAMPLE_ETHER_VALUE = ethers.parseEther("0.0000001");
const Height = 31; // The height of the Merkle tree used in EthShi, which is 2^31 leaves
...
async function main() {
    ...
    // Deploy EthShi contract
    const EthShiFactory = await ethers.getContractFactory("EthShi");
    const EthShi = await EthShiFactory.deploy(verifier1Address, verifier2Address, poseidon2Address, Height);
    await EthShi.waitForDeployment();
    const EthShiAddress = await EthShi.getAddress();
    console.log("EthShi deployed at:", EthShiAddress);

    // Measure gas usage for shm deposit insertions
    const insertionCount = INSERTION_COUNT;
    const imtGasUsed = [];

    for (let i = 0; i < insertionCount; i++) {
        const txResponse = await EthShi.deposit(SAMPLE_COMMITMENT, SAMPLE_DEPOSIT_VALUE, { value: SAMPLE_ETHER_VALUE });
        const receipt = await txResponse.wait();
        imtGasUsed.push(receipt.gasUsed.toString());
        printProgress(i + 1, insertionCount, "EthShi");
    }

    console.log("EthShi - Gas used per insert:", imtGasUsed);
    ...
    const outputFilePath = path.join(__dirname, "../data/shi_dep_31_gas_used.json");
